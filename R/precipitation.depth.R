...
#' 
#' @param x object of precipitation probability returned by \code{\link{precipitation.probability}} 
...
#' 
#' 
#' @seealso \code{\link{qqfit}},\code{\link{fitdistrForPrecipitation}},\code{\link{generate}},\code{\link{precipitation.probability}}
#' 
#' 
#' @examples 
#' 
...
## # 
## #prec_gen <- precipitation.depth(x=generation_monthly,origin=origin,par=fit_monthly)
## #
...
## #						names=names(prec_mes),precipitation.indicator=TRUE)
## # 
## #prec_gen_2 <- precipitation.depth(x=generation_monthly_2,origin=origin,par=fit_monthly)
## #
...
precipitation.depth <- function(x,origin="1961-1-1",par=NULL,...){
	...
	if (class(par)=="fitdistrForEachStation") {
		...
		for ( m in months) {
			...
			out[x$month==m,] <- precipitation.depth(x=x[x$month==m,stations],origin=NULL,par=par[[m]])
