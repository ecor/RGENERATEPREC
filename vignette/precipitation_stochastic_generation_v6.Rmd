---
title: "Stochastic Generation of Daily Precipitation"
...
Stochastic generators of weather variables, called "Weather Generators" (WGs) have been widely developed in the recent decades for hydrological and agricultural applications [@Richardson1981,@Racsko1991,@Semenov1997,@Parlange2000,@Liu2009,@Chen2012,@Chen2014]. A typical application of WGs is the reproduction of daily weather time series from downscaled monthly climate predictions [@Mearns2001,@Wilks1999,@Qian2002,@Semenov2010]. If high-resolution application models are to be used with downscaled series, a meteorological consistence of generated series is required, suggesting the use of a multi-site weather generator. Algorithms to represent historical spatial dependences of weather variables have been developed e.g. by @Wilks1998,@Khalili2009,@Serinaldi2009,@Bardossy2009. @Wilks1998 simulated rainfall occurrences through a generation of combinations of Gaussian random variables and established a relationship for each pair of rain gauges between Gaussian variables correlation and binary precipitation occurrence values. In this way, weather generators can reproduce at least partially spatial correlations. This approach is widely cited in literature @Mehrotra2006,@Brissette2007,@Serinaldi2009,@Thompson2007,@Mhanna2011.  
...
Package *RGENARATEPREC* and other necessary packages are loaded as follows:
...
The third element is the order of the autoregression, i.e. how many day before whise data are used for the regression in the genaralized linear model: 
...
In case of model is applied to a daily time series of precipitation occurrence and exogenous variale, i.e. predictors, they must be included as a new data frame equal to *newdata* argument: 
```{r eval=TRUE}
row_test <- 2000:2007
...
The  first item is a generalized linear model, in which Gaussianized value of precipitation depends on months: 
...
###   https://mgimond.github.io/ES218/Week05b.html
library(ggplot2)
library(lubridate)
library(reshape2)
...
class(model$ccgamma)
```
which is matrix of corresponding  Gaussian correlation of precipitation occurrances calculated with the algorithm introduced by  @Wilks1998 (related documentation under preperation); this method is used  alternatively to a complete logistic regression among stations that would have been computationally heavy;  

```{r eval=TRUE}
class(model$K)
```

which corresponds to the dimensionality (i.e. the number of the gauging stations);

```{r eval=TRUE}
class(model$type)
```

which is the type of the model: in this case, it is based on Wilks' approach to handle the plurality of the gauging stations;

```{r eval=TRUE}
class(model$station)
```

which is a vector of the names of the gauging stations;
...
which is the lag order of autoregression for the generalized linear model. 
...
The precipitation amount stochastic generator is then built as follows: firt, a model is created:
```{r eval=TRUE}
...
The  first are generalized linear models, in which Gaussianized values of precipitation depend on month of the year:
...
In conclusion, the spatial statistical coherence between observed an generated time-series can be checked making use of *CCGamma* function:
...
```
where *cc_mes* and *cc_gen* are list containing joint probability and correlation matrix among precipation occurrences for each couple of sites. In case of *sample=monthly* setting, these matrices replicated for each of the 12 months of the year. THe structures of  *cc_mes* and *cc_gen* can be displayed as follows:
...
## Adaptation on Precipitation Projection
...
###prec_val  <- split(df$obs,f=df$month) %>% lapply(FUN=function(x,valmin){x[x>valmin]},valmin=valmin)
```

```{r eval=TRUE,fig.width=7}
color_month <- rep(c("red","green","blue"),4)[c(12,1:11)]
google.cols <- yarrr::piratepal(palette = "google",trans = .2) 
color_month <- google.cols[color_month] %>% str_sub(1,7)
color_month <-  rep(c("#E6352F","#34A74B","#3D79F3" ),4)[c(12,1:11)]###"#E6352F" "#34A74B" "#3D79F3" "#E6352F" "#34A74B" "#3D79F3" "#E6352F"
##[12] "#34A74B"
...
Though L-momemnt method @lmom , the samples of daily precipitation are fitted with a LN3 (@Sangal1970) distribution, in most of months the null hypothesis of the Kolgormov-Smirnov test can be accepted  (*pvalue>0.1*):  
...
##prec_val is missing ## SOLVE THIS ISSUE!!!
...
<!-- https://stackoverflow.com/questions/6989015/ecdf-on-the-same-plot-using-ggplot2 -->

## Conclusions

An examples of the usage of the precipitation stochastic generation containd in **RGENERATEPREC** package has been showed. Qisualization through quantile-quantile plots and statistical testing (e.g. Kolgomorov-Smirnov testing) the similarity of  the stastistical properties (e.g. probability distrubtions of precipitation occurrence, precipitation account, dry spell, wet spell per each month of the year) between observed and generated time series can be chacked. The weather generator can work with one or more time series generating scenarios for several sites and mantainig their also spatial cross-correlation. This vignette has showed an example applied to some stations of *trentino* dataset. An example on how to use the weather generator with data having different probabilistic distribution (e.g. more climate variability) has been finally illustrated.

<!-- For historical resaon, *tidyverse* dialact were not used in this vigngette, the reader is free to use **RGENERATEPREC** and adpt the snippet of code towards **tidyverse** dialect. -->

<!-- This vignette shoes an example of a stochastic generation based on  a single observed time series in a rainfall gauging station and a generation of several time series based on a network of rainfall gauging stations. Therefore, an examples of stochastic generator of daily time series based on monthly climate future projections. The examples makes used of the example dataset already presented in **RMAWGEN** package, dataset already imported by **RGENERATEPREC**. -->
