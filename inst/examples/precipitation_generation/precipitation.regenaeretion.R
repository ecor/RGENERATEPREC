...
###source('/Users/ecor/R-packages/RGENERATEPREC/R/rearrange.precipitation.generation.R', chdir = TRUE)
valmin <- 1.0
fit_monthly <- fitdistrForPrecipitation(data=prec_mes,dname="weibull",start=NULL,sample="monthly",origin=origin)

prec_gen0  <- normalizeGaussian_severalstations(x=prec_gen_gaussWilks, data=prec_mes,sample="monthly",extremes=TRUE,origin_x = origin_sim, origin_data = origin,inverse=TRUE)

probs <- precipitation.probability(x=prec_gen_gaussWilks,data=prec_mes,valmin=valmin,origin_x=origin_sim,origin_data=origin,sample="monthly")

prec_gen <- precipitation.depth(x=probs,origin=origin_sim,par=fit_monthly)

prectempor <- prec_gen
prec_gen <- prec_gen0
prec_gen0 <- prectempor
...
source('/Users/ecor/R-packages/RGENERATEPREC/inst/examples/precipitation_generation/precipitation.analysis.R', chdir = TRUE)
...
#valmin=0.5
#qqplot(prec_mes[,12][prec_mes[,12]>valmin],prec_gen[,12][prec_gen[,12]>valmin])
#abline(0,1)
